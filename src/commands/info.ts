    ...
    async info(interaction: CommandInteraction) {
        ...
        const embed = new MessageEmbed()
            ...
            .addFields(
                ...
                { name: await lang("INFO_EXCHANGE_RATE_LABEL", interaction.user.id), value: await lang("INFO_EXCHANGE_RATE_VALUE", interaction.user.id, [ ticker, exchangeRate, currency, calculatedExchange ] )},
                { name: await lang("INFO_CURRENT_SUPPLY_LABEL", interaction.user.id), value: `${supply}`, inline: true },
                { name: await lang("INFO_BLOCK_REWARD_LABEL", interaction.user.id), value: `${blockReward}`, inline: true },
                { name: await lang("INFO_AMOUNT_MINED_LABEL", interaction.user.id), value: await lang("INFO_AMOUNT_MINED_VALUE", interaction.user.id, [ amountMined ] ) },
                { name: await lang("INFO_MARKET_CAP_LABEL", interaction.user.id), value: `${marketCap} ${currency}` },
                { name: await lang("INFO_FULLY_DILUTED_MARKET_CAP_LABEL", interaction.user.id), value: `${parseDecimals(exchangeRate * maxSupply)} ${currency}` },
